...
| `learn book 1 [题号]` | 浏览第一册：正确答案绿色高亮，Enter 下一题 / Ctrl+Enter 上一题 |
| `help` | 显示帮助 |
| `clear` | 重显标题 |
| `quit` | 退出 |
...
学习浏览（`learn`）模式：**无需作答**，正确答案以**绿色背景高亮**直接显示，`Enter` 下一题、`Ctrl+Enter` 上一题、`q` 退出。可配合题号跳转（如 `learn book 2 30`）从特定题开始浏览。
