...
| `learn book 1 [題號]` | 瀏覽第一冊：正確答案綠色高亮，Enter 下一題 / Ctrl+Enter 上一題 |
| `help` | 顯示幫助 |
| `clear` | 重顯標題 |
| `quit` | 退出 |
...
學習瀏覽（`learn`）模式：**不須作答**，正確答案以**綠色背景高亮**直接顯示，`Enter` 下一題、`Ctrl+Enter` 上一題、`q` 退出。可配合題號跳轉（如 `learn book 2 30`）從特定題開始瀏覽。
